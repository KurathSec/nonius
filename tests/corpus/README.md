...
**This is a fixture and not a benchmark.** It exists so that every decision the composer
makes can be checked against arithmetic a person can do on paper. It is not an item set anyone
...
whose numbers cannot be checked on paper is a snapshot rather than a calibration.
...
| `mem-a` | a bool result, which no slot here accepts, so bool tested before int matters |
